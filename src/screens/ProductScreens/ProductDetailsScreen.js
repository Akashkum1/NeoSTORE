...
const ProductDetails = (props) => {
            ...
            <View style={styles.indicatorView}>
                {productData.colors.map((colors, i) =>{
                    return(
                        <View  key={`color-${i}`}>
                            {i === 0 ?
                                <Text style={styles.colorText}> {colors.charAt(0).toUpperCase()+ colors.slice(1)}</Text> :
                                <Text style={styles.colorText}>, {colors.charAt(0).toUpperCase()+ colors.slice(1)}</Text>
                            }
                        </View>
                    )
    ...
    const onShare = async () => {
        try {
            const result = await Share.share({
                message: 'Checkout this product on NeoStore | ' + productData.name + ' ' + productData.description + '',
            });
            if (result.action === Share.sharedAction) {
                if (result.activityType) {
                    ToastAndroid.showWithGravityAndOffset("Successfully shared", ToastAndroid.LONG, ToastAndroid.BOTTOM, 25, 50);
                } else {
                    
                }
            } 
            else if (result.action === Share.dismissedAction) {
                ToastAndroid.showWithGravityAndOffset("User closed the share options", ToastAndroid.LONG, ToastAndroid.BOTTOM, 25, 50);
            }
        } 
        catch (error) {
            alert(error.message);
    ...
    const addCart = () => {
        if(checkedColor === null){
            ...
            ToastAndroid.showWithGravityAndOffset("Please select the Color", ToastAndroid.LONG, ToastAndroid.BOTTOM, 25, 50);
        }
        else{
            ...
            { productData ===null?
                ...
                <ScrollView contentContainerStyle={styles.main} showsVerticalScrollIndicator={false}>
                    <FlatList
                        ...
                        renderItem={({item}) =>{
                            return (
                                <View style={styles.productSliderView}>
                                    <Image 
                    ...
                    <View style={[{marginBottom:20},styles.returnView]}>
                        ...
                        <View style={styles.colorsAvailableView}>
                            <Text style={styles.colorsAvailableText}> Available in: </Text>
                            <ColorsAvailable/>
                        </View>
                    ...
                    <View style={styles.rateView}>
                        <Text style={styles.rateText}> {productData.rating} </Text>
                ...
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        <View style={styles.mainView}>
                            <Text onPress={() => { setShowmodal(false) }} style={styles.closeButton}>x</Text>
